import HasilReviewPenelitian from '@/components/HasilReviewPenelitian';
import HasilReviewPengabmas from '@/components/HasilReviewPengabmas';
import DashLayout from '@/layouts/dash-layout';
import { BreadcrumbItem } from '@/types';
import { Head } from '@inertiajs/react';
import { useState } from 'react';

const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Hasil Review',
        href: '/dosen/hasil-review',
    },
];

export default function HasilReview() {
    const [activeTab, setActiveTab] = useState('penelitian');

    const tabs = [
        { id: 'penelitian', label: 'Daftar Review Penelitian' },
        { id: 'pengabmas', label: 'Daftar Review Pengabdian Masyarakat' },
    ];

    return (
        <DashLayout breadcrumbs={breadcrumbs}>
            <Head title="ASIPP | Hasil Review" />
            <div className="rounded-lg bg-white p-6 shadow">
                {/* Tab Bar */}
                <h2 className="text-xl font-bold">Hasil Review</h2>
                <div className="flex border-b">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`flex-1 px-4 py-2 text-center font-medium transition ${
                                activeTab === tab.id ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600 hover:text-blue-500'
                            }`}
                        >
                            {tab.label}
                        </button>
                    ))}
                </div>

                {/* Isi Tab */}
                <div className="rounded-b border bg-white p-4">
                    {activeTab === 'penelitian' && <HasilReviewPenelitian></HasilReviewPenelitian>}
                    {activeTab === 'pengabmas' && <HasilReviewPengabmas></HasilReviewPengabmas>}
                </div>
            </div>
        </DashLayout>
    );
}
