...
export default function AturPlotingUsulanForm({ title }: { title: string }) {
    const {
        data,
        setData,
        // post,
    ...
    // dummy data
    const judulOptions = [
        { value: 'p1', label: 'Analisis Sistem Informasi Kampus' },
        { value: 'p2', label: 'Penerapan IoT untuk Smart Home' },
    ];
        ...
        <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center">
                    <Label className="w-40 font-semibold">Judul {title}*</Label>
                    <Select value={data.judul} onValueChange={(value) => setData('judul', value)}>
                        <SelectTrigger>
                            <SelectValue placeholder="- Pilih -" />
                        </SelectTrigger>
                        <SelectContent>
                            {judulOptions.map((item) => (
